...
export const DokterPage = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await GetKaryawan("Dokter");
            setData(res);
        } catch (error) {
    ...
    return (
        <div className="p-6">
            <Breadcrumbs size="lg" itemClasses={{ item:"text-slate-700 font-semibold data-[current=true]:text-slate-400 data-[current=true]:font-normal", separator:"text-slate-700 text-xl" }}>
                ...
                <BreadcrumbItem className="font-normal">Dokter</BreadcrumbItem>
            </Breadcrumbs>
                    ...
                    {loading ? (
                        ...
                        >
                            <TableHeader>
                                ...
                                <TableColumn>Spesialis</TableColumn>
                                <TableColumn>Aksi</TableColumn>
                            </TableHeader>
                            <TableBody items={items} emptyContent={"Tidak Ada Dokter"}>
                                {items.map((item, index) => (
                                    <TableRow key={item.nama}>
                                        ...
                                        <TableCell className="text-ellipsis whitespace-nowrap overflow-hidden max-w-[150px] md:max-w-none">{getKeyValue(item.dokter, "spesialisasi")}</TableCell>
