...
import { ModalTambahObat } from "@/components/modal/obat/Modal-tambah-obat";
import { ModalHapusObat } from "@/components/modal/obat/Modal-hapus-obat";
import { ModalUbahObat } from "@/components/modal/obat/Modal-ubah-obat";
          ...
          <CardBody>
            <div className="justify-between items-center hidden sm:flex">
              <div className="flex flex-row gap-2">
                <ModalTambahObat fetch={fetchData} />
              ...
              <TableBody items={items} emptyContent={"Tidak Ada Obat"}>
                {items.map((item, index) => (
                  <TableRow key={item.nama}>
                    ...
                    <TableCell>{getKeyValue(item, "satuan")}</TableCell>
                    <TableCell>
                      {formatRupiah(getKeyValue(item, "harga"))}
                    </TableCell>
                    <TableCell className="flex flex-row space-x-2 md:space-y-0">
                      <ModalHapusObat id={item.id} fetch={fetchData} />
                      <ModalUbahObat obat={item} fetch={fetchData} />
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
